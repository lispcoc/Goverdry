...
function debugCommand (command) {
  ...
  while (command.match(/\[(.*?)\]/)) {
    ...
    if (GoverdryPatch) {
      regAry.push(command.match(/\[(.*?)\]/)[1])
      reg = new RegExp(
        '\\[' + MP.escapeProcessing(command.match(/\[(.*?)\]/)[1]) + '\\]'
      )
    }
...
function enableChangeClassReplace (formula, race, sex, ali, abi, raceComment) {
  ...
  if (formula.indexOf('pc.abi0') > -1 && abi != undefined && abi != null) {
    ...
    while (formula.match(reg)) {
      ...
      if (GoverdryPatch) {
        num = Number(formula.match(reg)[1])
      }
...
function loopLoading () {
  console.log('[Goverdry]loopLoading', GameState)
...
function main (readEnd) {
  console.log('[Goverdry]main', readEnd)
  if (readEnd == 'webFont') {
      ...
      let sound = DOMSound.load(
        ...
        function (sound) {
          ...
          if (readFileCount[0] >= readFileName.length - 1) {
            ...
            readFileCount[0]++
            readFileCount[1] = 0
            console.log(readFileCount[0])
            console.log(readFileCount[1])
            readSoundFile2(soundName, readFileName, readFileCount, callbackFunc)
          }
        },
        function (sound) {
...
function replaceFormula (formula, repAry, noDice, noGm) {
  ...
  for (let i = 0; i < repAry.length; i++) {
    ...
    if (formula.indexOf(cStr + '.itemCommentNum') > -1) {
      if (cStr.match(/pc/)) {
        ...
        while (formula.match(reg)) {
          ...
          if (GoverdryPatch) {
            str2 = formula.match(reg)[1]
            num = Number(formula.match(reg)[2])
          }
          let str3 = replaceBrackets('de', str2)
    ...
    if (formula.indexOf(cStr + '.mpMax') > -1) {
      ...
      while (formula.match(reg)) {
        ...
        if (GoverdryPatch) {
          num1 = Number(formula.match(reg)[1])
          num2 = Number(formula.match(reg)[2])
        }
    ...
    if (formula.indexOf(cStr + '.MasteredSpell') > -1 && cStr.match(/pc/)) {
      ...
      while (formula.match(reg)) {
        ...
        if (GoverdryPatch) {
          kind = Number(formula.match(reg)[1])
          lv = Number(formula.match(reg)[2])
          spell = Number(formula.match(reg)[3])
        }
        let result = '0'
  ...
  if (pt > -1) {
    ...
    while (formula.match(reg)) {
      ...
      if (GoverdryPatch) {
        refStr = formula.match(reg)[1]
        num = Number(formula.match(reg)[2]) - 1
      }
      let result = 0
  ...
  if (formula.indexOf('DumpItem') > -1) {
    ...
    while (formula.match(reg)) {
      ...
      if (GoverdryPatch) {
        refStr = formula.match(reg)[1]
        num1 = Number(formula.match(reg)[2])
        num2 = Number(formula.match(reg)[3])
      }
      let stack = []
      for (let i = 0; i < PlayData['GarbageItem'].length; i++) {
  ...
  if (formula.indexOf('Wall') > -1) {
    ...
    while (formula.match(reg)) {
      ...
      if (GoverdryPatch) {
        num3 = Number(formula.match(reg)[4])
      }
      let result = 0
  ...
  if (formula.indexOf('Map') > -1) {
    ...
    while (formula.match(reg)) {
      ...
      if (GoverdryPatch) {
        num4 = Number(formula.match(reg)[5])
      }
      let result = 0
...
function replaceFormulaParty (pt, possibleToAction, refStr) {
  ...
  for (let i = 0; i < party.PartyMember.length; i++) {
    ...
    if (refStr.match(/itemCommentNum\[([^\[\]]+)\]\[([0-5])\]/)) {
      ...
      let num = Number(RegExp.$2)
      if (GoverdryPatch) {
        str = refStr.match(/itemCommentNum\[([^\[\]]+)\]\[([0-5])\]/)[1]
        num = Number(refStr.match(/itemCommentNum\[([^\[\]]+)\]\[([0-5])\]/)[2])
      }
      ...
      let str = RegExp.$1
      if (GoverdryPatch) {
        str = Number(refStr.match(/classCommentNum\[([^\[\]]+)\]/)[1])
      }
      let str2 = replaceBrackets('de', str)
      ...
      let str = RegExp.$1
      if (GoverdryPatch) {
        str = Number(refStr.match(/raceCommentNum\[([^\[\]]+)\]/)[1])
      }
      let str2 = replaceBrackets('de', str)
      ...
      let num = Number(RegExp.$1)
      if (GoverdryPatch) {
        num = Number(refStr.match(/Sex\[([0-3])\]/)[1])
      }
      if (num == PC[pc].Sex) {
        result = 1
        break
      ...
      let num = Number(RegExp.$1)
      if (GoverdryPatch) {
        num = Number(refStr.match(/Ali\[([0-2])\]/)[1])
      }
      ...
      let num = Number(RegExp.$1)
      if (GoverdryPatch) {
        num = Number(refStr.match(/Race\[([0-9]+)\]/)[1])
      }
      if (num == PC[pc].Race) {
        result = 1
        break
      ...
      let num = Number(RegExp.$1)
      if (GoverdryPatch) {
        num = Number(refStr.match(/Class\[([0-9]+)\]/)[1])
      }
      ...
      let num2 = Number(RegExp.$2)
      if (GoverdryPatch) {
        num1 = Number(refStr.match(/haveItemNum\[([0-9]+)\]\[([0-5])\]/)[1])
        num2 = Number(refStr.match(/haveItemNum\[([0-9]+)\]\[([0-5])\]/)[2])
      }
      result += PC[pc].haveItemNum(num1, num2)
    } else if (refStr.match(/Resist0\[([0-9]+)\]/)) {
      let num = Number(RegExp.$1)
      if (GoverdryPatch) {
        num = Number(refStr.match(/Resist0\[([0-9]+)\]/)[1])
      }
      ...
      let num = Number(RegExp.$1)
      if (GoverdryPatch) {
        num = Number(refStr.match(/Resist1\[([0-9]+)\]/)[1])
      }
      ...
      let num = Number(RegExp.$1)
      if (GoverdryPatch) {
        num = Number(refStr.match(/Resist\[([0-9]+)\]/)[1])
      }
      if (PC[pc].Resist[num]) {
        result = 1
        break
      ...
      if (GoverdryPatch) {
        kind = Number(
          refStr.match(/MasteredSpell\[([0-9]+)\]\[([0-9]+)\]\[([0-9]+)\]/)[1]
        )
        lv = Number(
          refStr.match(/MasteredSpell\[([0-9]+)\]\[([0-9]+)\]\[([0-9]+)\]/)[2]
        )
        spell = Number(
          refStr.match(/MasteredSpell\[([0-9]+)\]\[([0-9]+)\]\[([0-9]+)\]/)[3]
        )
      }
      ...
      let lv = Number(RegExp.$2)
      if (GoverdryPatch) {
        kind = Number(refStr.match(/MasteredSpell\[([0-9]+)\]\[([0-9]+)\]/)[1])
        lv = Number(refStr.match(/MasteredSpell\[([0-9]+)\]\[([0-9]+)\]/)[2])
      }
      let spell = -1
      if (PC[pc].spellMaster(kind, lv, spell)) {
        result = 1
      ...
      let kind = Number(RegExp.$1)
      if (GoverdryPatch) {
        kind = Number(refStr.match(/MasteredSpell\[([0-9]+)\]/)[1])
      }
      let lv = -1
      let spell = -1
      if (PC[pc].spellMaster(kind, lv, spell)) {
...
function replaceFormulaPartyAverage (pt, possibleToAction, refStr, noGm) {
  ...
  for (let i = 0; i < party.PartyMember.length; i++) {
    ...
    if (refStr.match(/lv|maxLv|minLv/)) {
      ...
      if (GoverdryPatch) {
        num = refStr.match(
          /abi0\[([0-9]+)\]|maxAbi0\[([0-9]+)\]|minAbi0\[([0-9]+)\]/
        )[1]
      }
      if (num >= GameData['ABILITY'].length) {
        num = 0
      ...
      if (GoverdryPatch) {
        num = refStr.match(
          /abi1\[([0-9]+)\]|maxAbi1\[([0-9]+)\]|minAbi1\[([0-9]+)\]/
        )[1]
      }
      if (num >= GameData['ABILITY'].length) {
        num = 0
...
function replaceGenericFormula (str) {
  ...
  while (str.match(GENERIC_FORMULA_REG)) {
    let num = Number(RegExp.$1)
    if (GoverdryPatch) {
      num = str.match(GENERIC_FORMULA_REG)[1]
    }
    let result = '0'
...
function saveData (text) {
  if (GoverdryPatch) {
    if (text != "Interruption of adventure" && localStorage.inSaveInterval()) {
      return
    }
    OVERLAY.setMessage('Saving...', 0.8, 0.03)
  }
  ...
  let dataStr = SAVE.getSaveDataStr(
    JSON.stringify({
      GameData: GameData,
      PlayData: PlayData,
      PC: PC,
...
function setGameData (base64Text) {
  ...
  if (LaunchSettings['GraphicPrefetch']) {
    ...
    while (sourceStr.match(reg)) {
      ...
      if (GoverdryPatch) {
        prefetchStr = 'graphic/' + sourceStr.match(reg)[1]
      }
...
function spellEffectReplace (effect, type) {
  ...
  while (effect.match(reg)) {
    ...
    let num = Number(RegExp.$2)
    if (GoverdryPatch) {
      refStr = effect.match(reg)[1]
      num = Number(effect.match(reg)[2])
    }
    let str = ''
