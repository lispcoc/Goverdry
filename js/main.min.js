...
function debugCommand (command) {
  ...
  while (command.match(/\[(.*?)\]/)) {
    ...
    if (GoverdryPatch) {
      ...
      reg = new RegExp('\\[' + MP.escapeProcessing(command.match(/\[(.*?)\]/)[1]) + '\\]')
    }
...
function replaceFormulaParty (pt, possibleToAction, refStr) {
  ...
  for (let i = 0; i < party.PartyMember.length; i++) {
    ...
    if (refStr.match(/itemCommentNum\[([^\[\]]+)\]\[([0-5])\]/)) {
      ...
      let spell = Number(RegExp.$3)
      if (GoverdryPatch) {
        kind = Number(refStr.match(/MasteredSpell\[([0-9]+)\]\[([0-9]+)\]\[([0-9]+)\]/)[1])
        lv = Number(refStr.match(/MasteredSpell\[([0-9]+)\]\[([0-9]+)\]\[([0-9]+)\]/)[2])
        spell = Number(refStr.match(/MasteredSpell\[([0-9]+)\]\[([0-9]+)\]\[([0-9]+)\]/)[3])
      }
...
function replaceFormulaPartyAverage (pt, possibleToAction, refStr, noGm) {
  ...
  for (let i = 0; i < party.PartyMember.length; i++) {
    ...
    if (refStr.match(/lv|maxLv|minLv/)) {
      ...
      let num = Number(RegExp.$1)
      if (GoverdryPatch) {
        num = refStr.match(/abi0\[([0-9]+)\]|maxAbi0\[([0-9]+)\]|minAbi0\[([0-9]+)\]/)[1]
      }
      if (num >= GameData['ABILITY'].length) {
        num = 0
      }
      ...
      let num = Number(RegExp.$1)
      if (GoverdryPatch) {
        num = refStr.match(/abi1\[([0-9]+)\]|maxAbi1\[([0-9]+)\]|minAbi1\[([0-9]+)\]/)[1]
      }
      if (num >= GameData['ABILITY'].length) {
        num = 0
      }
      ...
      let num = Number(RegExp.$1)
      if (GoverdryPatch) {
        num = refStr.match(/abi\[([0-9]+)\]|maxAbi\[([0-9]+)\]|minAbi\[([0-9]+)\]/)[1]
      }
      if (num >= GameData['ABILITY'].length) {
        num = 0
      }
      ...
      let num = Number(RegExp.$1)
      if (GoverdryPatch) {
        num = refStr.match(/gm\[([0-9]+)\]|maxGm\[([0-9]+)\]|minGm\[([0-9]+)\]/)[1]
      }
      if (num >= 10) {
        num = 0
      }
