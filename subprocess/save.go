...
func getSaveDataStr (ctx *quickjs.Context, this quickjs.Value, args []quickjs.Value) quickjs.Value {
	GameData_READ_KEYWORD = args[0].Get("READ_KEYWORD").String() //GameData
	PlayData_ShopItemListFull = args[1].Get("ShopItemListFull").Bool() //PlayData
	PlayData_GameFlag = args[1].Get("GameFlag") //PlayData

	var data = ""
	data += GameData_READ_KEYWORD
	data += '\n'
	if PlayData_ShopItemListFull {
		data += "1"
	}else{
		data += "0"
	}
	data += '\n'
	var num = 1
	for i := 1; i < PlayData_GameFlag.ToArray().Len(); i++ {
		if PlayData_GameFlag.ToArray().Get(int64(i)) {
			num = i
		}
	}
	if (num > 9999) {
